...
if not api_key:
    ...
    if owner_id:
        # Try creating service with ALL possible fields based on all error messages
        service_data = {
            "name": "todo-backend-test",
            "type": "web_service",
            "ownerId": owner_id,
            "repo": {
                "url": "https://github.com/danishyameen/todo-web-app-ai-chatbot.git",
                "branch": "main"
            },  # Try repo as object instead of string
            "serviceDetails": {
                "type": "web_service",  # Type also inside serviceDetails
                "runtime": "python",
                "planId": "starter",
                "region": "oregon",
                "autoDeploy": True,
                ...
                "branch": "main",
                "buildCommand": "pip install -r backend/requirements.txt",  # Also inside serviceDetails
